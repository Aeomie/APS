...
let print_argp argp = 
  match argp with
  | ASTArgP(name,t) -> (
    Printf.printf"ASTArgP(%s,"name;
    print_type t;
    Printf.printf")";
  )
  | ASTArgPAddress(name,t) -> (
    Printf.printf"ASTArgPAddress(%s,"name;
    print_type t;
    Printf.printf")";
  )
;;
...
let rec print_argsp argsp = 
  match argsp with
  | [] -> ()
  | [argp] -> print_argp argp
  | argp::rest_argsp -> (
    print_argp argp;
    print_char ',';
    print_argsp rest_argsp;
  )
;;
let rec print_expr e =
  ...
  | ASTNth(vec,expr) -> (
    Printf.printf"ASTNth(";
    print_expr vec;
    print_char ',';
    print_expr expr;
    print_char ')';
  )
  | ASTVset(vec,pos,value) -> (
    Printf.printf"ASTVset(";
    print_expr vec;
    print_char ',';
    print_expr pos;
    print_char ',';
...
and print_exprp exprp = 
  match exprp with
  | ASTExpr(e) -> (
    Printf.printf"ASTExpr(";
    print_expr e;
    Printf.printf")";
  )
  | ASTExprAddress(ident) -> (
    Printf.printf"ASTExprAddress (%s)"ident
  )

and print_exprsp exprsp = 
  match exprsp with
  | [] -> ()
  | [exprp] -> print_exprp exprp
  | exprp::rest_exprsp -> (
    print_exprp exprp;
    print_char ',';
    print_exprsp rest_exprsp;
  )
...
and print_lval v =
  match v with
  | ASTLval(ident) -> Printf.printf"ASTId(%s)"ident
  | ASTNthLval(var, value) -> (
    Printf.printf"ASTNthLval(";
    print_lval var;
    print_char ',';
    print_expr value;
    print_char ')';
